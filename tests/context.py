...
class TestContext(object):
    def __init__(self):
        ...
        _path = "%s/%s" % (self.server_url, self.post_path)
        self.device_config = DeviceConfig.download(_path, post_key=self.post_key)
        self.sensor_id = "FriskPITest_PM10"

        self.device_config_broken = DeviceConfig.download(_path, post_key=self.post_key)
